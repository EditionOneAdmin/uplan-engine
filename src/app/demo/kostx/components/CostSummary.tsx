  ...
  return (
    <div className="sticky top-0 z-10 bg-white/[0.02] backdrop-blur-sm border-b border-white/10 p-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2">
        <div className={`rounded-xl p-3 border ${gesamtM2 < 3500 ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-red-500/30 bg-red-500/5'}`}>
          <div className="text-[10px] text-white/50 uppercase tracking-wider mb-1">KG 300+400 gesamt</div>
          <div className="flex items-baseline gap-1">
            <span className={`text-lg font-bold ${gesamtM2 < 3500 ? 'text-emerald-400' : 'text-red-400'}`}>{fmt(gesamtM2)}</span>
            <span className="text-xs text-white/40">€/m²</span>
          </div>
          {result.basement && <div className="text-[10px] text-white/40 mt-0.5">davon TG-Mehrkosten: {fmt(tgM2)} €/m²</div>}
        </div>
